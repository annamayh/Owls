...
n_distinct(check$BirdId)


```

Ids with unknown death date

```{r}
dead_noRing_noDd=All_after2010_noRing%>%
  filter(BornClutchId==RaisedClutchId)%>%
  filter(DeathDate=='')
...
parents_of_inbred1s=inbred_clutches%>%
  ...
  pivot_longer(cols = c(MaleRing, FemaleRing), values_to = "RingId")%>%#putting parents into one col
  filter(CrossFostered==0) ## also add na 
