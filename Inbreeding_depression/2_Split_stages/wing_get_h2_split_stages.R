...
h2_split=readRDS("Inbreeding_depression_owls/Model_outputs/2_split_stages/2.3.Wing_split_stage_h2.RDS")
summary(h2_split)
plot(h2_split)
...
h.bill.juv <- as.mcmc((Var.table$sd_RingId__gr_stageJuvenile)^2 / ((Var.table$sd_RingId__gr_stageJuvenile)^2 + (Var.table$sd_RingId_pe__gr_stageJuvenile)^2+(Var.table$sd_month__gr_stageJuvenile)^2 + (Var.table$sd_year__gr_stageJuvenile)^2 + 
                                                                     (Var.table$sd_clutch_merge__gr_stageJuvenile)^2 + (Var.table$sd_nestboxID__gr_stageJuvenile)^2 + (Var.table$sd_nestboxID__gr_stageJuvenile)^2 + (Var.table$b_sigma_gr_stageJuvenile)^2))
summary(h.bill.juv)
...
h.bill.adu <- as.mcmc((Var.table$sd_RingId__gr_stageAdult)^2 / ((Var.table$sd_RingId__gr_stageAdult)^2 + (Var.table$sd_RingId_pe__gr_stageAdult)^2+(Var.table$sd_month__gr_stageAdult)^2 + (Var.table$sd_year__gr_stageAdult)^2 + 
                                                                 (Var.table$sd_clutch_merge__gr_stageAdult)^2 + (Var.table$sd_nestboxID__gr_stageAdult)^2  + (Var.table$b_sigma_gr_stageAdult)^2))
...
total.var.juv=as.mcmc((Var.table$sd_RingId__gr_stageJuvenile)^2 + (Var.table$sd_RingId_pe__gr_stageJuvenile)^2+(Var.table$sd_month__gr_stageJuvenile)^2 + (Var.table$sd_year__gr_stageJuvenile)^2 + 
                       (Var.table$sd_clutch_merge__gr_stageJuvenile)^2  + (Var.table$sd_nestboxID__gr_stageJuvenile)^2 + (Var.table$b_sigma_gr_stageJuvenile)^2)
...
rands=c("sd_RingId_" , "sd_RingId_pe_", "sd_year_", "sd_month_", "sd_nestboxID_" , "sd_clutch_merge_", "b_sigma")
...
variance_exp_per_stage=rbind(juve.var, adult.var)%>%
  mutate(effect=as.factor(effect)%>%
           fct_recode(Va="sd_RingId_", 
                      ...
                      resid="b_sigma")%>%
           fct_relevel("resid", "year", "month", "nestbox", "clutch" ,"pe", "Va")
