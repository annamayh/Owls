...
prior_mass_gr<- c(
  prior(normal(360, 10), nlpar = "asym",  class="b"),##
  prior(normal(4, 0.5), nlpar = "b",  class="b"), ## 
  prior(normal(0.89, 0.2), nlpar = "c",  class="b"), ## 
  
  prior(student_t(3, 0, 60), class = "sigma", lb=0),
  prior(student_t(3, 0, 60), class="sd",nlpar = "asym", lb=0), # 
  prior(student_t(3, 0, 60),  class="sd", nlpar = "b", lb=0),
  prior(student_t(3, 0, 60),  class="sd", nlpar = "c", lb=0), 
...
saveRDS(growth_mass.mod,file="./outputs/3_growth/3.2_mass_unscaled_gr_15k.RDS") ##
