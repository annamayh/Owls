...
fledge_tarsus_df=read.table("fledge_pheno_tarsus_df.txt",sep=",", header=T)
...
## ### Fgrm simple model ### ###
#################################
mod_tarsus.1.2.FROH <- MCMCglmm(LeftTarsus ~  1 + FHBD512gen +GeneticSex+rank+SSgompertz(age_days, 719.5, 2.274, 0.89),
...
ped=read.table("pedigreeCORRECTED.tab", header = T)%>%
  select(-sex)%>%
  rename(RingId=id)
...
mod_tarsus.2.ped.FROH <- MCMCglmm(LeftTarsus ~  1 + FHBD512gen+GeneticSex+rank+SSgompertz(age_days, 719.5, 2.274, 0.89),
...
plot(mod_tarsus.2.ped.FROH)
summary(mod_tarsus.2.ped.FROH)

autocorr.diag(mod_tarsus.2.ped.FROH$VCV)

##################################################################################
### MASS ###

fledge_mass_df=read.table("fledge_pheno_mass_df.txt",sep=",", header=T)
...
## ### FROH simple model ### ###
#################################
mod_mass.1.2.FROH <- MCMCglmm(Mass ~  1 + FHBD512gen +GeneticSex+rank+SSgompertz(age_days, 3.695e+02, 4.425,8.906e-01),
                           random = ~RingId+Observer+clutch_merge+year, 
                           data = fledge_mass_df,
                           prior = prior1.2, 
                           nitt = 80000, 
...
plot(mod_mass.1.2.FROH)
summary(mod_mass.1.2.FROH)
...
mod_mass.2.ped.FROH <- MCMCglmm(Mass ~  1 + FHBD512gen+GeneticSex+rank+SSgompertz(age_days, 3.695e+02, 4.425,8.906e-01),
                             random = ~RingId+RingId_pe+Observer+clutch_merge+year, 
                             data = fledge_mass_df,
                             ginv = list(RingId = Ainv),
                             prior = prior1.3, 
...
plot(mod_mass.2.ped.FROH)
summary(mod_mass.2.ped.FROH)

###############################################################################
### Bill Length ########

fledge_bill_df=read.table("fledge_pheno_bill_df.txt",sep=",", header=T)
...
mod_bill.1.2.FROH <- MCMCglmm(BillLength ~  1 + FHBD512gen +GeneticSex+rank+SSgompertz(age_days, 1.798e+02, 1.127, 9.205e-01),
                           random = ~RingId+Observer+clutch_merge+year, 
                           data = fledge_bill_df,
                           prior = prior1.2, 
                           nitt = 80000, 
...
mod_bill.2.ped.FROH <- MCMCglmm(BillLength ~  1 + FHBD512gen+GeneticSex+rank+SSgompertz(age_days, 1.798e+02, 1.127, 9.205e-01),
                           random = ~RingId+RingId_pe+Observer+clutch_merge+year, 
                           data = fledge_bill_df,
                           ginv = list(RingId = Ainv),
                           prior = prior1.3, 
...
plot(mod_bill.2.ped.FROH)
summary(mod_bill.2.ped.FROH)



save(mod_tarsus.1.2.FROH, mod_tarsus.2.ped.FROH,
     mod_mass.1.2.FROH, mod_mass.2.ped.FROH,
     mod_bill.1.2.FROH,mod_bill.2.ped.FROH,
     file="FROH_all_model_outputs_Juveniles.RData")
